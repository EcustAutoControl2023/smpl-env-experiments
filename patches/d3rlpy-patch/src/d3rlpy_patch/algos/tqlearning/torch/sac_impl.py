...
class SACIFModules(DDPGBaseModules):
    policy: NormalPolicy
    log_temp: Parameter
...
@dataclasses.dataclass(frozen=True)
class SACIFActorLoss(DDPGBaseActorLoss):
    temp: torch.Tensor
    temp_loss: torch.Tensor


class SACIFImpl(DDPGBaseImpl):
    _modules: SACIFModules
...
@dataclasses.dataclass(frozen=True)
class DiscreteSACIFModules(Modules):
...
class DiscreteSACIFImpl(DiscreteQFunctionMixin, QLearningAlgoImplBase):
    _modules: DiscreteSACIFModules
