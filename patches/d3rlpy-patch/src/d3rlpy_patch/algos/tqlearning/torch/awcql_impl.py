...
from d3rlpy.algos.qlearning.torch.sac_impl import SACImpl, SACActorLoss, SACModules
from d3rlpy.algos.qlearning.torch.utility import sample_q_values_with_policy

__all__ = ["AWCQLImpl", "DiscreteAWCQLImpl", "AWCQLModules", "DiscreteAWCQLLoss"]


@dataclasses.dataclass(frozen=True)
class AWCQLModules(SACModules):
    log_alpha: Parameter
    alpha_optim: Optional[OptimizerWrapper]


@dataclasses.dataclass(frozen=True)
class AWCQLCriticLoss(DDPGBaseCriticLoss):
    conservative_loss: torch.Tensor
    alpha: torch.Tensor


class AWCQLImpl(SACImpl):
    _modules: AWCQLModules
...
@dataclasses.dataclass(frozen=True)
class DiscreteAWCQLLoss(DQNLoss):
    td_loss: torch.Tensor
    conservative_loss: torch.Tensor


class DiscreteAWCQLImpl(DoubleDQNImpl):
    _alpha: float
