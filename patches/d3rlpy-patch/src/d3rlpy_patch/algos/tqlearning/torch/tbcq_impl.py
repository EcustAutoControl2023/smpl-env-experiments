...
from d3rlpy.algos.qlearning.torch.dqn_impl import DoubleDQNImpl, DQNLoss, DQNModules
...
class TBCQModules(DDPGBaseModules):
...
class TBCQImpl(DDPGBaseImpl):
    ...
    def compute_actor_grad(self, batch: TorchMiniBatch) -> DDPGBaseActorLoss:
        ...
        latent = torch.randn(batch_size, 2 * self._action_size, device=self._device)
    ...
    def compute_imitator_grad(self, batch: TorchMiniBatch) -> dict[str, torch.Tensor]:
    ...
    def _sample_repeated_action(
        ...
        return action.squashed_mu.view(-1, self._n_action_samples, self._action_size)
...
@dataclasses.dataclass(frozen=True)
class DiscreteTBCQModules(DQNModules):
    imitator: CategoricalPolicy


@dataclasses.dataclass(frozen=True)
class DiscreteTBCQLoss(DQNLoss):
    td_loss: torch.Tensor
    imitator_loss: torch.Tensor


class DiscreteTBCQImpl(DoubleDQNImpl):
    _modules: DiscreteTBCQModules
    _action_flexibility: float
    _beta: float
